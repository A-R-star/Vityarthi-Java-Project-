...
## LIBRARY MANAGEMENT SYSTEM – A COMMAND-LINE JAVA APPLICATION
...
### 2. ABSTRACT
The Library Management System is a robust command-line application designed to manage core library operations, including book inventory, member registration, issue/return transactions, fine calculations, data export, and automated multi-user synchronization. Developed using Java 17, Maven, SQLite, JDBC, JPA/Hibernate, and JUnit 5, the application provides a modular software architecture demonstrating Object-Oriented Programming (OOP) principles, custom exception handling, thread-safe concurrent transactions, character/byte I/O streams, and hybrid database persistence via JDBC and JPA.
...
- `com. library.annotation.CourseConcept`: Custom annotation mapping code elements to architectural components.
- `com. library.model.Book`: Abstract base class implementing `Borrowable`, `Searchable`, and `Exportable`.
- `com. library.model.PhysicalBook`: Concrete subclass for borrowable physical inventory.
- `com. library.model.ReferenceBook`: Concrete subclass for non-borrowable reading room items.
- `com. library.model.User`: Abstract base class for library users.
- `com. library.model.Student`: Concrete subclass defining student borrowing quota (3 books).
- `com. library.model.Librarian`: Concrete subclass defining librarian borrowing quota (10 books).
- `com. library.model.LibraryTransaction`: Transaction entity containing static nested summary class.
- `com. library.util.DatabaseManager`: Singleton connection manager and SQLite table initializer.
- `com. library.util.JpaUtil`: Singleton `EntityManagerFactory` manager.
- `com. library.util.InputValidator`: Exception-safe console input reader.
- `com. library.repository.BookJdbcRepository`: JDBC DAO for book operations.
- `com. library.repository.UserJdbcRepository`: JDBC DAO for user operations.
- `com. library.repository.TransactionJdbcRepository`: JDBC DAO for transaction records.
- `com. library.repository.JpaLibraryRepository`: JPA DAO executing JPQL queries.
- `com. library.service.LibraryService`: Service facade providing thread-safe business methods.
- `com. library.thread.ConcurrentCheckoutTask`: Runnable task for thread testing.
- `com. library.thread.MultithreadingDemoManager`: Concurrency runner demonstrating synchronized lock behavior.
- `com. library.reflection.ReflectionInspector`: Reflection API dynamic class inspector.
- `com. library.io.ReportExporter`: Character stream report exporter.
- `com. library.io.BackupManager`: Byte stream binary backup utility.
- `com. library.ui.ConsoleMenu`: Command-line loop controller.
...
### 24. RESULTS
The application successfully builds into a single fat JAR artifact (` -library-management-1.0-SNAPSHOT-jar-with-dependencies.jar`). All 14 CLI menu features, database persistences, thread synchronization locks, file stream exports, and reflection inspections function without errors.
...
### 28. CONCLUSION
The   Library Management System demonstrates a complete, working, core Java implementation. By combining Object-Oriented principles, multithreaded synchronization, custom exceptions, Collections, file streams, JDBC, and JPA Hibernate ORM, the project provides a real-world software solution.
